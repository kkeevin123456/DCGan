...
class GAN:
	...
	def build_model(self):
		...
		# disc_real_image, disc_real_image_logits   = self.discriminator(DiffAugment(t_real_image, policy=policy), t_real_caption)
		# disc_wrong_image, disc_wrong_image_logits   = self.discriminator(DiffAugment(t_wrong_image, policy=policy), t_real_caption, reuse = True)
		# disc_fake_image, disc_fake_image_logits   = self.discriminator(DiffAugment(fake_image, policy=policy), t_real_caption, reuse = True)
	...
	def generator(self, t_z, t_text_embedding):
		...
		reduced_text_embedding = ops.lrelu( ops.linear(t_text_embedding, self.options['t_dim'], 'g_embedding') )
		# print(t_z.shape, reduced_text_embedding.shape) # -> (5, 100) (5, 256)
		z_concat = tf.concat([t_z, reduced_text_embedding], 1)
		z_ = ops.linear(z_concat, self.options['gf_dim']*8*s16*s16, 'g_h0_lin')
		h0 = tf.reshape(z_, [-1, s16, s16, self.options['gf_dim'] * 8])
